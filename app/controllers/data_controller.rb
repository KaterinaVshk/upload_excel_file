...
class DataController < ApplicationController
  ...
  def show
    id_file = @datum.id
    @getting_data = Records.new.get_data(id_file)
    @classes = NamesClass.all
  end
  ...
  def create
    ...
    respond_to do |format|
      if @datum.save
        id = @datum.id
        @data = ParseData.new.get_inform(datum_params,id)
        format.html { redirect_to @datum, notice: "Данные успешно загружены" }
  ...
  private
    ...
    def datum_params
      params.require(:datum).permit(:name,:file_attch)
    end
end
